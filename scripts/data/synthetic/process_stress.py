#!/usr/bin/env python3
"""
Process all audio files from synth_extended.jsonl with stress analysis.
...
INPUT_JSONL = _this_dir / "synth_extended.jsonl"
OUTPUT_JSONL = _this_dir / "synth_stress.jsonl"
    ...
    with open(OUTPUT_JSONL, 'w', encoding='utf-8') as fout:
        for i, entry in enumerate(tqdm(data, desc="Processing stress analysis")):
            ...
            if speech_rec and "text" in speech_rec:
                ...
                if transcript:
                    print(f"\n[{i+1}/{len(data)}] Processing: {entry['relative_audio_path']} (using pre-generated transcript)")
                else:
                    print(f"\n[{i+1}/{len(data)}] Processing: {entry['relative_audio_path']} (no transcript available)")
            else:
                print(f"\n[{i+1}/{len(data)}] Processing: {entry['relative_audio_path']} (no speech_recognition in tool_outputs)")
