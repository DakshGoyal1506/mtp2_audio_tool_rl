...
from grpo.dataset import split_mmau_dataset
from grpo.model_wrapper import DeSTA25GRPOModel
from grpo.modeling_grpo import GRPODeSTA25AudioModel, _prepare_audio_context_and_start_positions
from grpo.prompts import build_grpo_initial_prompt, build_grpo_followup_prompt
from grpo.rewards_trl import trl_reward_function, set_reward_weights
            ...
            for item in self.eval_items:
                ...
                with torch.no_grad():
                    comps = desta_wrapper.generate_completions(
                        ...
                        followup_user_template=build_grpo_followup_prompt,
                        question=question,
                        choices=choices,
                        G=1,
                        temperature=0.0,
                ...
                comp = comps[0]
                from grpo.rewards_trl import extract_answer
...
class AudioTRLGRPOTrainer(GRPOTrainer):
    ...
    """
    def __init__(self, *args, kl_threshold: float = 0.0, **kwargs):
    ...
    def _generate(self, prompts: List[Any]):
        ...
        for audio_path, indices in unique_groups.items():
            ...
            # Phase 1 & 2: generate with custom wrapper
            comps = desta_wrapper.generate_completions(
                audio_path=audio_path,
                system_prompt=sys_p,
                user_prompt=user_p,
            ...
            messages_p1 = desta_wrapper.build_messages_phase1(
                audio_path, sys_p, user_p, transcription=transcription, precomputed_embed=precomputed_embed
            )
            ...
            for j, idx in enumerate(indices):
                ...
                if comp["called_tools"]:
                    phase1_ids = tokenizer(comp["phase1"], return_tensors="pt", add_special_tokens=False).input_ids[0].tolist()
                    injected_ids = tokenizer(comp["injected_output"], return_tensors="pt", add_special_tokens=False).input_ids[0].tolist()
                    phase2_ids = tokenizer(comp["phase2"], return_tensors="pt", add_special_tokens=False).input_ids[0].tolist()
                    
                    completion_ids = phase1_ids + injected_ids + phase2_ids
                    # 1 = model token, 0 = tool injection (do not penalize/train on these)
                    tool_mask = [1] * len(phase1_ids) + [0] * len(injected_ids) + [1] * len(phase2_ids)
                else:
