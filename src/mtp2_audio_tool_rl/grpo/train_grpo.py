...
from grpo2.dataset       import split_mmau_dataset, GRPOAudioDataset
from grpo2.model_wrapper import DeSTA25GRPOModel
from grpo2.model_wrapper_qwen import QwenOmniGRPOModel
from grpo2.trainer       import GRPOTrainer
...
def load_desta_model(cfg, device):
    ...
    try:
        from grpo2.modeling_grpo import GRPODeSTA25AudioModel  # type: ignore
    except ImportError:
    ...
    model.to(device)
    model._setup_generation()  # Ensure tokenizer/processor are loaded
...
def main():
    ...
    if use_qwen:
        from grpo2.modeling_qwen_grpo import load_qwen_model, enable_lora_qwen
        ...
        ref_model, _ = load_qwen_model(cfg, device)
        ref_model.eval()
        for p in ref_model.parameters():
            p.requires_grad = False
        ref_wrapper = QwenOmniGRPOModel(ref_model, qwen_processor, device)
    else:
        policy_model = load_desta_model(cfg, device)
        policy_wrapper = DeSTA25GRPOModel(policy_model, device)
        
        # Load reference model (frozen, no LoRA)
        # We need to reload the base model without LoRA.
        # Note: load_desta_model calls _enable_lora. We want to skip that for ref.
        ...
        ref_model = GRPODeSTA25AudioModel.from_pretrained(
            cfg.model_hf_name,
            torch_dtype=torch.bfloat16,
        )
        ref_model.to(device)
        ref_model._setup_generation()  # Ensure tokenizer/processor are loaded
        ref_model.eval()
        for p in ref_model.parameters():
            p.requires_grad = False
        ref_wrapper = DeSTA25GRPOModel(ref_model, device)
    ...
    # We DO NOT prepare the reference model - it stays on the device we put it on
