...
def trl_reward_function(prompts, completions, **kwargs) -> List[float]:
    ...
    for i in range(len(completions)):
        ...
        # Count tool calls from JSON arrays inside <tool>...</tool>
        ...
        if c_score >= 0.7:
            ...
            if not called_tools:
                tool_score = 1.0    # Best: correct without tool (highly efficient)
            else:
                # Good: correct with tool.
                if tool_count == 1:
                    tool_score = 0.8
                else:
                    # Diminishing returns for spamming tools before getting it right
                    tool_score = max(0.0, 0.8 - (0.2 * (tool_count - 1)))
        else:
            # Got the answer WRONG
            if called_tools:
                # Penalty for calling tools but failing. Capped at -1.0 to
                # prevent extreme negative advantages from tool-spam.
        ...
        # Clamp reward to [0, 1] to bound advantage magnitude and prevent
        # extreme policy gradients that cause KL/entropy divergence.
