        ...
        for q_idx, (item, completions) in enumerate(zip(batch, rollout_data)):
            ...
            for comp in completions:
                ...
                if comp["called_tools"]:
                    ...
                    ref_lp_p1, _ = self.policy.reference_log_probs(messages_p1, comp["phase1"])
                    ref_lp_p2, _ = self.policy.reference_log_probs(messages_p2, comp["phase2"])
                    ref_lp = (ref_lp_p1 * n1 + ref_lp_p2 * n2) / total_tokens

                else:
                    policy_lp, _ = self.policy.compute_log_probs(messages_p1, comp["phase1"])
                    ref_lp, _    = self.policy.reference_log_probs(messages_p1, comp["phase1"])
